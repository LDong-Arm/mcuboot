...
int flash_area_read_is_empty(const struct flash_area* fap, uint32_t off, void* dst, uint32_t len) {

    if (flash_area_read(fap, off, dst, len)) {
        MCUBOOT_LOG_ERR("flash_area_read error: fa_id %d offset 0x%x len 0x%x", fap->fa_id, off, len);
